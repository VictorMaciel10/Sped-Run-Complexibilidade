...
from ..models import Question, Option, GameSession, Answer, User
from ..schemas import (
    LaunchResponse,
    QuestionOut,
    AnswerIn,
...
def answer(
    ...
    correct_opt = session_db.exec(
        select(Option).where(
            ...
            Option.is_correct == True,  # noqa: E712
        )
    ).first()
...
def finalize(
    ...
    answers = session_db.exec(
        select(Answer).where(Answer.session_id == gs.id)
    ).all()
...
# ------------ Histórico da última sessão do usuário ------------ #
@router.get("/last-session", response_model=LastSessionOut)
def get_last_session(
    session_db: Session = Depends(get_session),
    user: User = Depends(get_current_user),
):
    """
    Retorna a ÚLTIMA GameSession do usuário (maior id),
    junto com o somatório dos tempos das respostas.
    """
    gs = (
        session_db.exec(
            select(GameSession)
    ...
    total_time_ms = sum(a.elapsed_ms for a in answers)
