...
# ------------ NOVO: último histórico do usuário ------------ #
@router.get("/last-session", response_model=LastSessionOut)
def get_last_session(
    session_db: Session = Depends(get_session),
    user: User = Depends(get_current_user),
):
    # Última sessão finalizada (finished_at != None)
    gs = (
        session_db.exec(
            select(GameSession)
            .where(
                GameSession.user_id == user.id,
                GameSession.finished_at != None,  # noqa: E711
            )
    ...
    if not gs:
        # Usuário ainda não tem sessão finalizada
