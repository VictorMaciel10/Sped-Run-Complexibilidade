...
async function register(){
  ...
  // em geral o /auth/register não retorna token, mas deixei como estava
...
async function login(){
  ...
  // carrega o último histórico assim que logar
...
async function finalize(){
  ...
  // depois de finalizar, atualiza o histórico da última sessão
  await loadLastSession();
}
...
async function loadLastSession(){
  ...
  const dateSpan = document.getElementById('last-date');

  if (!j.session_id){
    scoreSpan.textContent = '-';
    timeSpan.textContent = '-';
    dateSpan.textContent = 'sem histórico';
    return;
  }

  scoreSpan.textContent = j.total_score;
  timeSpan.textContent = j.total_time_ms;
  dateSpan.textContent = j.finished_at
    ? new Date(j.finished_at).toLocaleString('pt-BR')
    : '-';
}